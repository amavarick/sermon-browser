...
function sb_install() {
	...
	$single_tpl = strtr(sb_default_single_template(), sb_sermon_page_dictionary());
	$single_tpl = str_replace('implode($ref_output, ", ")', 'implode(", ", $ref_output)', (string)$single_tpl);
	sb_update_option('single_output', $single_tpl);

	$search_tpl = strtr(sb_default_multi_template(), sb_search_results_dictionary());
	$search_tpl = str_replace('implode($ref_output, ", ")', 'implode(", ", $ref_output)', (string)$search_tpl);
	sb_update_option('search_output', $search_tpl);

	$books = sb_get_default('eng_bible_books');
	foreach ((array)$books as $book) {
		$wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}sb_books (name) VALUES (%s)", $book));
	}
...
div.sermon-browser table, div.sermon-browser td { border: none; }
div.sermon-browser tr td { padding: 4px 0; }
div.sermon-browser table.podcast table { margin: 0 1em; }
div.sermon-browser td.sermon-title, div.sermon-browser td.sermon-passage { font-family: "Helvetica Neue",Arial,sans-serif; }
...
div.sermon-browser table.sermons td.preacher { border-bottom: 1px solid #444; padding-bottom: 1em; }
...
table.sermonbrowser td { white-space: nowrap; padding: 5px; }
...
.podcastall { float:left; background: #fff0c8; padding: 0.5em; border-radius: 7px; }
.podcastcustom { float:right; background: #fce4ff; padding: 0.5em; border-radius: 7px; }
...
<div class="sermon-browser">
	...
	[sermons_loop]
		...
		<tr><td class="sermon-passage">[first_passage] (Part of the [series_link] series).</td></tr>
		<tr><td class="files">[files_loop][file][/files_loop]</td></tr>
		<tr><td class="embed">[embed_loop][embed][/embed_loop]</td></tr>
		<tr><td class="preacher">Preached by [preacher_link] on [date] ([service_link]).</td></tr>
	[/sermons_loop]
	</table>
</div>
HERE;
